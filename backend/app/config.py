...
only at startup, to create the tables and refresh vocabulary and practice
items) and an *app* role for everything a request does. ``DB_URL`` carries only
host/port/database — credentials and the async driver are injected per role.
...
DEFAULT_DB_URL = 'postgresql://localhost:5432/conjugation'
